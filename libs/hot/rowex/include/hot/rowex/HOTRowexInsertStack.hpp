...
struct HOTRowexInsertStack {
  ...
  HOTRowexFirstInsertLevel<EntryType> determineInsertLevel(
      hot::commons::DiscriminativeBit const& mismatchingBit) {
    ...
    // it must be inserted into the child node this is an alternative approach
    // to using getLeastSignificantDiscriminativeBitForEntry
