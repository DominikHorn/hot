...
template <>
__attribute__((always_inline)) inline auto toBigEndianByteOrder<uint64_t>(
    uint64_t const &key) {
  return __bswap_64(key);
};

template <>
inline auto toBigEndianByteOrder<uint32_t>(uint32_t const &key) {
  return __bswap_32(key);
};

template <>
inline auto toBigEndianByteOrder<uint16_t>(uint16_t const &key) {
...
template <>
constexpr inline size_t getMaxKeyLength<char const *>() {
 ...
 */
template <typename KeyType>
inline size_t getKeyLength(KeyType const &key) {
  return getMaxKeyLength<KeyType>();
}

template <>
inline size_t getKeyLength<char const *>(char const *const &key) {
 ...
 */
template <>
inline auto toFixSizedKey(char const *const &key) {
 ...
 */
template <typename KeyType>
__attribute__((always_inline)) inline uint8_t const *interpretAsByteArray(
    KeyType const &key) {
  return reinterpret_cast<uint8_t const *>(&key);
}

template <>
inline uint8_t const *interpretAsByteArray<const char *>(
    const char *const &cStringKey) {
